...
export default class Upload {
    ...
    static convertLocalIdentifierToAssetLibrary(localIdentifier, ext) {
        const hash = localIdentifier.split('/')[0];
        return `assets-library://asset/asset.${ext}?id=${hash}&ext=${ext}`;
    };

    static async upload(files, replace = true) {
        const data = new FormData();
        let result = false
        files.forEach((e, i) => {
            let uri = e.uri
            if (Platform.OS === 'ios' && replace == true) {
                uri = Upload.convertLocalIdentifierToAssetLibrary(uri.replace('ph://', ''), "jpg")
            }
            data.append(i, {
                uri: uri,
        ...
        fetch(`http://${config.ip}:${config.port}/upload`, {
            method: 'POST',
            body: data
        }).then(res => {
            result = true
    ...
    static async share(images) {
        if (await Sharing.isAvailableAsync()) {
            images.forEach(async e => {
                ...
                console.log("a1: ",await Sharing.shareAsync(uri, { UTI: "public.jpeg" }))
                return true
            })
